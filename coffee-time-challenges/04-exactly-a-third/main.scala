        ...
        for (c <- digits.combinations(4)) {
            for(d <- c.permutations) {
                ...
                for((digit, place) <- d.zipWithIndex) {
                    numerator += digit * scala.math.pow(10, place).toInt;
                }
                ...
                var cdigits = numerator.toString + denominator.toString;
                var cdigits_list = cdigits.toCharArray.distinct;

                // Print solution
                if (cdigits_list.length == 9 && !cdigits_list.contains('0')){
                    println("%d / %d = 1/3".format(numerator, denominator));
                }
            }
        }
    }
